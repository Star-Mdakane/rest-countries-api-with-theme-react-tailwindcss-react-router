...
const SearchInput = () => {
    ...
    return (
        <div className="flex px-8 gap-6 w-85.75 h-12 md:w-120 items-center  bg-white text-text dark:bg-secondary dark:text-white rounded-[5px] shadow-[0_2px_9px_0_rgb(0_0_0/5.32%)]">
            ...
            <input type="text" name="country-search" id="country-search" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full h-fit" placeholder="Search by country..." />
        </div>
    )
}
